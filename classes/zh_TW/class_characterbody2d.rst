...
目前速度向量，單位為圖元每秒，呼叫 :ref:`move_and_slide()<class_CharacterBody2D_method_move_and_slide>` 期間會進行使用並修改。
...
根據 :ref:`velocity<class_CharacterBody2D_property_velocity>` 移動該物體。該物體如果與其他物體發生碰撞，則會沿著對方滑動（預設只在地板上滑動），不會立即停止移動。如果對方是 **CharacterBody2D** 或 :ref:`RigidBody2D<class_RigidBody2D>`\ ，還會受到對方運動的影響。可以用於製作移動、旋轉的平臺，也可用於推動其他節點。

發生滑動碰撞時會改變 :ref:`velocity<class_CharacterBody2D_property_velocity>`\ 。要獲取最後一次碰撞，請呼叫 :ref:`get_last_slide_collision()<class_CharacterBody2D_method_get_last_slide_collision>`\ ，要獲取碰撞的更多資訊，請使用 :ref:`get_slide_collision()<class_CharacterBody2D_method_get_slide_collision>`\ 。

該物體接觸到移動平臺時，平臺的速度會自動加入到該物體的運動中。平臺運動所造成的碰撞始終為所有滑動碰撞中的第一個。

通用行為和可用屬性會根據 :ref:`motion_mode<class_CharacterBody2D_property_motion_mode>` 發生改變。
