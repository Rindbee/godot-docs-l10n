...
Restituisce l'angolo tra il nodo e il punto ``point`` in radianti.
...
Ruota il nodo in modo che il suo asse +X locale punti verso il punto ``point``, che dovrebbe utilizzare coordinate globali.

\ ``point`` non dovrebbe essere uguale alla posizione del nodo, altrimenti il nodo guarda sempre a destra.
...
Applica una rotazione al nodo, in radianti, a partire dalla sua rotazione attuale.
...
Trasla il nodo in base a ``offset`` specificato in coordinate locali.
