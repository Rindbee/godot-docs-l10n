...
Indica che una proprietà :ref:`int<class_int>` o :ref:`float<class_float>` dovrebbe essere all'interno di un intervallo specificato tramite la stringa indicativa ``"min,max"`` o ``"min,max,step"``. La stringa indicativa può, facoltativamente, includere ``"or_greater"`` e/o ``"or_lesser"`` per permettere un input manuale che vada rispettivamente sopra il massimo o sotto il minimo dei valori.

\ **Esempio:** ``"-360,360,1,or_greater,or_lesser"``.

Inoltre, è possibile includere altre parole chiave: ``"exp"`` per modificare in un intervallo esponenziale, ``"radians_as_degrees"`` per modificare angoli in radianti visualizzati in gradi (anche i valori dell'intervallo sono in gradi), ``"degrees"`` per indicare un angolo e ``"hide_slider"`` per nascondere lo slider.
...
Indica che una proprietà :ref:`int<class_int>` o :ref:`String<class_String>` sia un valore innumerato da scegliere tra una lista specificata tramite una stringa indicativa.
...
Indica che una proprietà :ref:`String<class_String>` può essere un valore enumerato da scegliere in una lista specificata tramite una stringa indicativa come ``"Hello,Something,Else"``.

A differenza di :ref:`PROPERTY_HINT_ENUM<class_@GlobalScope_constant_PROPERTY_HINT_ENUM>`, una proprietà con questa indicazione accetta comunque valori arbitrari e può essere vuota. La lista dei valori serve a indicare i valori possibili.
...
Indica che una proprietà :ref:`int<class_int>` è una maschera di bit con flag di bit con nomi.

La stringa indicativa è un elenco separato da virgole di nomi come ``"Bit0,Bit1,Bit2,Bit3"``. Gli spazi vuoti **non** sono rimossi da entrambe le estremità di un nome. Il primo nome nella lista ha un valore 1, il prossimo 2, poi 4, 8, 16 e così via. I valori espliciti possono anche essere specificati appendendo ``:integer`` al nome, ad esempio ``"A:4,B:8,C:16"``. Puoi anche combinare diversi flag (``"A:4,B:8,AB:12,C:16"``).
...
Indica che una proprietà :ref:`String<class_String>` è testo con interruzioni di riga. Modificarla mostrerà un campo di input di testo in cui è possibile digitare interruzioni di riga.
...
Restituisce il tipo interno della variabile ``variable``, attraverso i valori di :ref:`Variant.Type<enum_@GlobalScope_Variant.Type>`.
