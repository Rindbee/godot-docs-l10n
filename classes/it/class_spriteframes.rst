...
Restituisce ``true`` se l'animazione fornita è configurata a ripetersi quando termina la riproduzione. Altrimenti, restituisce ``false``.
...
Se ``loop`` è ``true``, l'animazione ``anim`` sarà riprodotta in ripetizione quando raggiunge la fine, oppure l'inizio se è riprodotta al contrario.
