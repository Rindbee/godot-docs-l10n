...
**Obsolète :** Use multiple :ref:`TileMapLayer<class_TileMapLayer>` nodes instead. To convert a TileMap to a set of TileMapLayer nodes, open the TileMap bottom panel with the node selected, click the toolbox icon in the top-right corner and choose 'Extract TileMap layers as individual TileMapLayer nodes'.
...
Émis lorsque le :ref:`TileSet<class_TileSet>` de cette TileMap change.
...
Efface les cellules qui n'existent plus dans le tileset.
...
**Obsolète :** Use :ref:`notify_runtime_tile_data_update()<class_TileMap_method_notify_runtime_tile_data_update>` and/or :ref:`update_internals()<class_TileMap_method_update_internals>` instead.
...
Renvoie le nombre de couches dans la TileMap.
...
Supprime la couche à l'index ``layer``.
