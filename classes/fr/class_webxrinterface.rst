...
Une interface XR utilisant WebXR.
...
Émis après que le taux de rafraîchissement de l'affichage a changé.
...
Émis lorsque :ref:`visibility_state<class_WebXRInterface_property_visibility_state>` modifié.
...
Renvoie le mode du rayon de la cible pour le ``input_source_id`` spécifié.

Cela peut aider à interpréter les entrées provenant de ce contrôleur. Voir `XRInputSource.targetRayMode <https://developer.mozilla.org/en-US/docs/Web/API/XRInputSource/targetRayMode>`__ (en anglais) pour plus d'informations.
