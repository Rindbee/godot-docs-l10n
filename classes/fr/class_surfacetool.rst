...
Un outil d'aide pour créer du géométrie.
...
- :doc:`Utiliser le SurfaceTool <../tutorials/3d/procedural_geometry/surfacetool>`
...
Utilisé pour indiquer un canal personnalisé désactivé.
...
Spécifie la position de l'actuel sommet. Doit être appelé après avoir spécifié les autres propriétés du sommet (par ex. la couleur, l'UV).
...
Appelé avant d'ajouter des sommets. Cela prend le type primitif comme argument (par exemple :ref:`Mesh.PRIMITIVE_TRIANGLES<class_Mesh_constant_PRIMITIVE_TRIANGLES>`).
...
Efface toutes les informations passées à l'outil de surface jusque là.
...
Crée un tableau de sommets depuis un :ref:`Mesh<class_Mesh>` existant.
...
Supprime le tableau d'indices en augmentant celui des sommets.
...
**Obsolète :** This method is unused internally, as it does not preserve normals or UVs. Consider using :ref:`ImporterMesh.generate_lods()<class_ImporterMesh_method_generate_lods>` instead.
...
Définit le :ref:`Material<class_Material>` à utiliser pour le :ref:`Mesh<class_Mesh>` qui vous construisez.
...
Spécifie une normale à utiliser pour le sommet *suivant*. Si chaque sommet a besoin d'avoir cette information définie et que vous ne l'avez pas fournie pour le premier sommet, cette information peut ne jamais être utilisée.
...
Spécifie une tangente à utiliser pour le sommet *suivant*. Si chaque sommet a besoin d'avoir cette information définie et que vous ne l'avez pas fournie pour le premier sommet, cette information peut ne jamais être utilisée.
...
Spécifie les coordonnées UV à utiliser pour le sommet *suivant*. Si chaque sommet a besoin d'avoir cette information définie et que vous ne l'avez pas fournie pour le premier sommet, cette information peut ne jamais être utilisée.
...
Spécifie les coordonnées UV secondaires facultatives à utiliser pour le sommet *suivant*. Si chaque sommet a besoin d'avoir cette information définie et que vous ne l'avez pas fournie pour le premier sommet, cette information peut ne jamais être utilisée.
