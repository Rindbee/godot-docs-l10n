...
Renvoie le :ref:`Vector2<class_Vector2>` à l'``index`` donné dans le tableau. Si ``index`` est hors des limites ou négatif, cette méthode échoue et renvoie ``Vector2(0, 0)``.
