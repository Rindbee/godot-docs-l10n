...
Calcule une fonction Step dans le graphe de shader visuel.
...
Sera traduit en ``step(edge, x)`` dans le code du shader.

Renvoie ``0.0`` si ``x`` est inférieur à ``edge``, et ``1.0`` sinon.
...
Représente la taille de l'énumération :ref:`OpType<enum_VisualShaderNodeStep_OpType>`.
