...
Renvoie le :ref:`Vector3<class_Vector3>` à l'``index`` donné dans le tableau. Si ``index`` est hors des limites ou négatif, cette méthode échoue et renvoie ``Vector3(0, 0, 0)``.
