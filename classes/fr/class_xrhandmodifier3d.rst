...
Un nœud pour contrôler des maillage de main des données :ref:`XRHandTracker<class_XRHandTracker>`.
...
Ce nœud utilise des données de suivi des mains d'un :ref:`XRHandTracker<class_XRHandTracker>` pour rendre les poses du squelette d'un maillage de main.

Le positionnement des mains est effectué en créant un ancêtre :ref:`XRNode3D<class_XRNode3D>` du maillage de main contrôlé par le même :ref:`XRHandTracker<class_XRHandTracker>`.

Les données de position du suivi des mains sont dimensionnées par :ref:`Skeleton3D.motion_scale<class_Skeleton3D_property_motion_scale>` lorsqu'elles sont appliquées au squelette, ce qui peut être utilisé pour ajuster la main suivie afin de correspondre à l'échelle du modèle de main.
...
Représente la taille de l'énumération :ref:`BoneUpdate<enum_XRHandModifier3D_BoneUpdate>`.
...
Le nom du :ref:`XRHandTracker<class_XRHandTracker>` enregistré avec :ref:`XRServer<class_XRServer>` d'où obtenir les données de suivi des mains.
