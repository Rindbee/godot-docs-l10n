...
Compte tenu d'un corps ``body``, d'une forme ``shape`` et de leurs paramètres respectifs, cette méthode devrait renvoyer ``true`` si une collision entre les deux se devrait se produire, avec les détails supplémentaires passés dans ``results``.

Version redéfinissable de la méthode interne ``shape_collide`` de :ref:`PhysicsServer2D<class_PhysicsServer2D>`. Correspond à :ref:`PhysicsDirectSpaceState2D.collide_shape()<class_PhysicsDirectSpaceState2D_method_collide_shape>`.
...
Compte tenu de deux formes et de leurs paramètres, devrait renvoyer ``true`` si une collision entre les deux pourrait se produire, avec des détails supplémentaires passés dans ``results``.
