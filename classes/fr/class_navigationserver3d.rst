...
- :doc:`Utilisation de NavigationServer <../tutorials/navigation/navigation_using_navigationservers>`
...
Émis quand une carte de navigation est mise à jour, ou quand une région se déplace ou est modifiée.
...
Définit le masque de bits du ``avoidance_layers`` de l'agent.
...
Définit le masque de bits ``avoidance_mask`` de l'agent.
...
Met à jour la hauteur ``height`` de l'``agent`` fourni.
...
Définit le nombre maximal d'autres agents que cet agent prend en compte dans la navigation. Plus ce nombre est grand, plus la durée de la simulation est longue. Si le nombre est trop bas, la simulation ne sera pas fiable.
...
Définit la vitesse maximale de l'agent. Ça doit être positif.
...
Définit la distance maximale avec d'autres agents que cet agent prend en compte dans la navigation. Plus ce nombre est grand, plus la durée de la simulation est longue. Si le nombre est trop bas, la simulation ne sera pas fiable.
...
Définit la position de l'agent dans l'espace global.
...
Renvoie ``true`` lorsque le NavigationServer a le débogage activé.
...
Renvoie tous les :ref:`RID<class_RID>` des cartes de navigation créées sur le NavigationServer. Cela renvoie les cartes de navigation 2D et aussi 3D car il n'y a techniquement aucune distinction entre elles.
...
Renvoie des informations sur l'état actuel du NavigationServer.
...
Crée un nouveau lien entre deux positions sur une carte.
...
Renvoie ``true`` si le lien ``link`` spécifié est activé.
...
Renvoie la position finale de ce lien ``link``.
...
Renvoie le coût d'entrée de ce lien ``link``.
...
Renvoie l'identifiant d'itération actuelle du lien de navigation. Chaque fois que le lien de navigation change et se synchronise, l'ID d'itération augmente. Un identifiant d'itération de ``0`` signifie que le lien de navigation n'a jamais synchronisé.

\ **Note :** L'ID d'itération bouclera à ``1`` après avoir atteint la limite de sa plage.
...
Renvoie les couches de navigation pour ce lien ``link``.
...
Renvoie la position de départ de ce lien ``link``.
...
Renvoie le coût de voyage de ce lien ``link``.
...
Définit la position de sortie pour le lien ``link``.
...
Définit le cout d'entrée ``enter_cost`` pour ce lien ``link``.
...
Définit la position d'entrée pour ce lien ``link``.
...
Définit le coût de voyage ``travel_cost`` pour ce lien ``link``.
...
**Obsolète :** This method is no longer supported, as it is incompatible with asynchronous updates. It can only be used in a single-threaded context, at your own risk.
...
Renvoie la marge de raccordement du bord de la carte. Cette distance est la distance minimale nécessaire pour relier deux bords de différentes régions.
...
Renvoie la direction haut de la carte.
...
Renvoie ``true`` si la carte est active.
...
Définit la marge de connexion de bord de la carte utilisée pour fusionner les bords compatibles de la région.
...
Définit la direction haut de la carte.
...
Définit le masque de bits du ``avoidance_layers`` de l'obstacle.
...
Renvoie le coût d'entrée de cette ``region``.
...
Renvoie les couches de navigation de la région.
...
Renvoie le coût de voyage de cette ``region``.
...
Définit le coût d'entrée ``enter_cost`` pour cette ``region``.
...
Définit le polygone de navigation de la région.
...
Définit l'``ObjectID`` de l'objet qui gère cette région.
...
Définit la transformation globale de cette région.
...
Définit le coût de voyage ``travel_cost`` pour cette ``region``.
...
Contrôle l'activation de ce serveur.
