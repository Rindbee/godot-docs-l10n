...
- :doc:`L'animation des sprites 2D (et aussi 3D) <../tutorials/2d/2d_sprite_animation>`
...
Émis lorsque :ref:`animation<class_AnimatedSprite3D_property_animation>` change.
...
Émis lorsque :ref:`frame<class_AnimatedSprite3D_property_frame>` change.
...
Émis lorsque :ref:`sprite_frames<class_AnimatedSprite3D_property_sprite_frames>` change.
...
L'animation actuelle de la ressource :ref:`sprite_frames<class_AnimatedSprite3D_property_sprite_frames>`. Si cette valeur est modifiée, le compteur :ref:`frame<class_AnimatedSprite3D_property_frame>` et :ref:`frame_progress<class_AnimatedSprite3D_property_frame_progress>` sont réinitialisés.
...
L'index de la trame d'animation affichée. Définir cette propriété réinitialise aussi :ref:`frame_progress<class_AnimatedSprite3D_property_frame_progress>`. Si cela n'est pas souhaité, utilisez :ref:`set_frame_and_progress()<class_AnimatedSprite3D_method_set_frame_and_progress>`.
...
Renvoie la vitesse réelle de lecture de l'animation actuelle ou ``0`` si elle n'est pas en train de jouer. Cette vitesse est la propriété :ref:`speed_scale<class_AnimatedSprite3D_property_speed_scale>` multipliée par l'argument ``custom_speed`` spécifié lors de l'appel de la méthode :ref:`play()<class_AnimatedSprite3D_method_play>`.
...
Renvoie ``true`` si une animation est jouée actuellement (même si :ref:`speed_scale<class_AnimatedSprite3D_property_speed_scale>` et/ou ``custom_speed`` valent ``0``).
...
Pause l'animation en cours de lecture. La trame :ref:`frame<class_AnimatedSprite3D_property_frame>` et :ref:`frame_progress<class_AnimatedSprite3D_property_frame_progress>` seront conservés et appeler :ref:`play()<class_AnimatedSprite3D_method_play>` ou :ref:`play_backwards()<class_AnimatedSprite3D_method_play_backwards>` sans arguments reprendra l'animation à la position de lecture actuelle.

Voir aussi :ref:`stop()<class_AnimatedSprite3D_method_stop>`.
...
Joue l'animation avec la clé ``name``. Si ``custom_speed`` est négatif et ``from_end`` vaut ``true``, l'animation jouera à l'envers (ce qui est équivalent à appeler :ref:`play_backwards()<class_AnimatedSprite3D_method_play_backwards>`).
...
Cette méthode est un raccourci pour :ref:`play()<class_AnimatedSprite3D_method_play>` avec ``custom_speed = -1.0`` et ``from_end = true``, voir donc sa description pour plus d'informations.
...
Définit :ref:`frame<class_AnimatedSprite3D_property_frame>` et :ref:`frame_progress<class_AnimatedSprite3D_property_frame_progress>` aux valeurs données. Contrairement à définir :ref:`frame<class_AnimatedSprite3D_property_frame>`, cette méthode ne réinitialise pas implicitement :ref:`frame_progress<class_AnimatedSprite3D_property_frame_progress>` à ``0.0``.

\ **Exemple :** Changer l'animation tout en gardant la même :ref:`frame<class_AnimatedSprite3D_property_frame>` et :ref:`frame_progress<class_AnimatedSprite3D_property_frame_progress>`\  :
...
Arrête l'animation en cours de lecture. La position d'animation est réinitialisée à ``0`` et la vitesse ``custom_speed`` est réinitialisée à ``1.0``. Voir aussi :ref:`pause()<class_AnimatedSprite3D_method_pause>`.
