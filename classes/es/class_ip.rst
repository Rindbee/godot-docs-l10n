...
IP contiene funciones de soporte para el Protocolo de Internet (IP). El soporte de TCP/IP está en diferentes clases (véase :ref:`StreamPeerTCP<class_StreamPeerTCP>` y :ref:`TCPServer<class_TCPServer>`). IP proporciona soporte para la resolución de nombres de host DNS, tanto bloqueante como en hilos.
...
El máximo número de consultas de resolución DNS concurrentes permitidas, :ref:`RESOLVER_INVALID_ID<class_IP_constant_RESOLVER_INVALID_ID>` se devuelve si se excede.
...
Constante de ID no válida. Se devuelve si se excede el valor :ref:`RESOLVER_MAX_QUERIES<class_IP_constant_RESOLVER_MAX_QUERIES>`.
...
Devuelve las direcciones resueltas, o un array vacío si se produjo un error o la resolución aún no se ha producido (véase :ref:`get_resolve_item_status()<class_IP_method_get_resolve_item_status>`).
...
Devuelve la dirección IPv4 o IPv6 de un nombre de host dado cuando se resuelve (método de tipo bloqueante). El tipo de dirección devuelto depende de la constante :ref:`Type<enum_IP_Type>` dada como ``ip_type``.
...
Crea un elemento de cola para resolver un nombre de host a una dirección IPv4 o IPv6 dependiendo de la constante :ref:`Type<enum_IP_Type>` dada como ``ip_type``. Devuelve el ID de la cola si tiene éxito, o :ref:`RESOLVER_INVALID_ID<class_IP_constant_RESOLVER_INVALID_ID>` en caso de error.
