...
.. XML source: https://github.com/godotengine/godot/tree/4.4/doc/classes/Skeleton3D.xml.
...
返回 ``bone_idx`` 骨骼是否存在键名为 ``key`` 的元数据。
...
将 ``bone_idx`` 骨骼中键名为 ``key`` 的元数据设置为 ``value``\ 。
