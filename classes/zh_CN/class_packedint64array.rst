...
.. XML source: https://github.com/godotengine/godot/tree/4.4/doc/classes/PackedInt64Array.xml.
...
\ **紧缩数组、类型化数组和非类型化数组之间的差异：**\ 与同类型的类型化数组相比，紧缩数组的迭代和修改速度通常更快（例如 **PackedInt64Array** 与 ``Array[int]``\ ）。此外，紧缩数组消耗的内存更少。缺点是，紧缩数组不太灵活，因为它们不提供许多便利方法，例如 :ref:`Array.map()<class_Array_method_map>`\ 。反过来，类型化数组的迭代和修改速度比非类型化数组更快。
...
返回数组中位于给定索引 ``index`` 处的 64 位整数。与使用 ``[]`` 运算符相同（\ ``array[index]``\ ）。
