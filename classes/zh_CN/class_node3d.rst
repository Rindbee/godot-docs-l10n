...
.. XML source: https://github.com/godotengine/godot/tree/4.4/doc/classes/Node3D.xml.
...
如果节点存在于 :ref:`SceneTree<class_SceneTree>` 中，并且 :ref:`visible<class_Node3D_property_visible>` 属性为 ``true``\ 、所有祖级节点也都可见，则返回 ``true``\ 。如果存在隐藏的祖级节点，则该节点在场景树中不可见。

父节点派生自 **Node3D** 时才会进行可见性检查。如果父节点为其他类型（例如 :ref:`Node<class_Node>`\ 、\ :ref:`AnimationPlayer<class_AnimationPlayer>`\ 、\ :ref:`Node2D<class_Node2D>`\ ），则会当作可见。

\ **注意：**\ 该方法不会考虑 :ref:`VisualInstance3D.layers<class_VisualInstance3D_property_layers>`\ ，因此即便返回 ``true``\ ，最后也可能不渲染该节点。
...
\ ``target`` 位置不能和该节点的位置相同，\ ``up`` 向量不能为零。

\ ``target`` 和 ``up`` 不能为 :ref:`Vector3.ZERO<class_Vector3_constant_ZERO>`\ ，两者也不能共线，这样可以避免围绕局部 Z 轴发生预料之外的旋转。
