...
如果节点存在于 :ref:`SceneTree<class_SceneTree>` 中，其 :ref:`visible<class_CanvasItem_property_visible>` 属性为 ``true``\ ，并且其所有祖先也可见，则返回 ``true``\ 。如果任何祖先被隐藏，则该节点在场景树中将不可见，因此也不会被绘制（参见 :ref:`_draw<class_CanvasItem_private_method__draw>`\ ）。

可见性仅在从 **CanvasItem**\ 、\ :ref:`CanvasLayer<class_CanvasLayer>` 和 :ref:`Window<class_Window>` 继承的父节点中检查。如果父节点是任何其他类型（例如 :ref:`Node<class_Node>`\ 、\ :ref:`AnimationPlayer<class_AnimationPlayer>` 或 :ref:`Node3D<class_Node3D>`\ ），则假定它是可见的。
