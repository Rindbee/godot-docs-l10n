...
.. XML source: https://github.com/godotengine/godot/tree/4.4/doc/classes/EditorExportPreset.xml.
...
导出预设配置。编辑器 UI 中的 **EditorExportPreset** 实例，旨在用作导出项目时传入 :ref:`EditorExportPlatform<class_EditorExportPlatform>` 方法的只读配置。
...
返回一个 :ref:`Dictionary<class_Dictionary>`\ ，包含在导出对话框的“资源”选项卡中选择的文件。字典的键是文件名，值是导出模式 ：\ ``"strip"``\ 、\ ``"keep"`` 或 ``"remove"``\ 。另见 :ref:`get_file_export_mode()<class_EditorExportPreset_method_get_file_export_mode>`\ 。
