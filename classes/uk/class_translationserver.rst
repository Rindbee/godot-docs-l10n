...
Повертає рядок ``locale``, стандартизований відповідно до відомих локалей (наприклад, ``en-US`` відповідатиме ``en_US``). Якщо ``add_defaults`` має значення ``true``, можливо, до локалі додано сценарій за умовчанням або країну.
