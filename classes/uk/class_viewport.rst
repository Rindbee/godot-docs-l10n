...
Не виконує жодних дій, якщо поточний елемент :ref:`Environment.sdfgi_enabled<class_Environment_property_sdfgi_enabled>` середовища має значення ``false``.
...
Встановлює режим фільтра за замовчуванням, який використовується :ref:`CanvasItem<class_CanvasItem>` у цьому вікні перегляду.
...
Встановлює режим повторення за замовчуванням, який використовується :ref:`CanvasItem<class_CanvasItem>` у цьому вікні перегляду.
...
Якщо ``true``, перегляд портів повинен надати свій фон як прозорий.
...
Якщо значення ``true``, використовується швидкий фільтр постобробки, щоб зробити смуги значно менш помітними. Якщо значення :ref:`use_hdr_2d<class_Viewport_property_use_hdr_2d>` дорівнює значенню ``false``, 2D-рендеринг *не* зазнає впливу дебендингу, якщо тільки значення :ref:`Environment.background_mode<class_Environment_property_background_mode>` не дорівнює значенню :ref:`Environment.BG_CANVAS<class_Environment_constant_BG_CANVAS>`. Якщо значення :ref:`use_hdr_2d<class_Viewport_property_use_hdr_2d>` дорівнює значенню ``true``, дебендинг застосовуватиметься лише в тому випадку, якщо це кореневий **Viewport**, і вплине на весь 2D- та 3D-рендеринг, включаючи елементи полотна.
...
Див. також :ref:`ProjectSettings.rendering/anti_aliasing/quality/use_debanding<class_ProjectSettings_property_rendering/anti_aliasing/quality/use_debanding>` та :ref:`RenderingServer.viewport_set_use_debanding()<class_RenderingServer_method_viewport_set_use_debanding>`.
...
\ **Примітка:** При спробі зберегти поточну текстуру (наприклад, у файлі) вона може бути повністю чорною або застарілою, якщо її використовувати занадто рано, особливо при використанні, наприклад, у :ref:`Node._ready()<class_Node_private_method__ready>`. Щоб переконатися, що отримана текстура є правильною, можна дочекатися сигналу :ref:`RenderingServer.frame_post_draw<class_RenderingServer_signal_frame_post_draw>`.
...
\ **Примітка:** Коли :ref:`use_hdr_2d<class_Viewport_property_use_hdr_2d>` має значення ``true``, повернена текстура буде HDR-зображенням, закодоване в лінійному просторі.
...
Зупиняє поширення вхідних даних далі вниз по :ref:`SceneTree<class_SceneTree>`.
