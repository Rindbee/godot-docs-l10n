...
\ **Примітка:** Якщо ви просто хочете знати, чи містить рядок ``what``, використовуйте :ref:`contains()<class_StringName_method_contains>`. У GDScript ви також можете використовувати оператор ``in``.
...
Видаляє будь-які символи, що зустрічаються в ``chars``. Див. також :ref:`remove_char()<class_StringName_method_remove_char>`.
...
Повертає індекс **останнього** входження ``what`` у цьому рядку, або ``-1``, якщо таких немає. Початок пошуку можна вказати за допомогою ``from``, продовжуючи до початку рядка. Цей метод є зворотним до :ref:`find()<class_StringName_method_find>`.
