...
Повертає кут між вузлом та ``point`` у радіанах.
...
Повертає вузол так, щоб його локальна вісь +X була спрямована до точки ``point``, яка, як очікується, використовуватиме глобальні координати.

\ ``point`` не повинен збігатися з позицією вузла, інакше вузол завжди дивиться праворуч.
...
Застосує локальний переклад на осі X вершини на основі :ref:`Node._process()<class_Node_private_method__process>` ``delta``. ``false``, нормалізує рух.
...
Наведено локальний переклад на вісь вузла на основі :ref:`Node._process()<class_Node_private_method__process>` ``delta``. ``false``, нормалізує рух.
...
Застосовується обертання в вузол, в радіях, починаючи від її поточного обертання.
...
Зміщує вузол на заданий ``offset`` у локальних координатах.
