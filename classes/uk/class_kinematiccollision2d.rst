...
Зберігає дані про зіткнення, отримані в результаті руху :ref:`PhysicsBody2D<class_PhysicsBody2D>`.
...
Утримує дані зіткнення з руху :ref:`PhysicsBody2D<class_PhysicsBody2D>`, як правило, від :ref:`PhysicsBody2D.move_and_collide()<class_PhysicsBody2D_method_move_and_collide>`. Коли переміщається :ref:`PhysicsBody2D<class_PhysicsBody2D>`, вона зупиняється, якщо виявить зіткнення з іншим тілом. Якщо виявлений зіткнення, повертається об'єкт **KinematicCollision2D**.
...
Повертає кут зіткнень за умовчанням ``up_direction``, який :ref:`Vector2.UP<class_Vector2_constant_UP>`. Це значення завжди позитивне.
...
Повертає прикріплені тіла, що зіштовхуються :ref:`Object<class_Object>`.
...
Повертає унікальний ідентифікатор екземпляра приєднаного до тіла, що зіштовхується, об'єкта :ref:`Object<class_Object>`. Див. метод ``Object.get_instance_id``.
...
Повернутися до корпусу, який використовується :ref:`PhysicsServer2D<class_PhysicsServer2D>`.
...
Повернення індексу форми тіла. :ref:`CollisionObject2D<class_CollisionObject2D>`.
