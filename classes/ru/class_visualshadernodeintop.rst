...
Применяет :ref:`operator<class_VisualShaderNodeIntOp_property_operator>` к двум целочисленным входам: ``a`` и ``b``.
...
Представляет размер перечисления :ref:`Operator<enum_VisualShaderNodeIntOp_Operator>`.
