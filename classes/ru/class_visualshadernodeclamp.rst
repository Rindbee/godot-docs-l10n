...
Фиксирует значение в графе визуального шейдера.
...
Ограничивает значение, значениями между ``min`` и ``max``.
...
Представляет размер перечисления :ref:`OpType<enum_VisualShaderNodeClamp_OpType>`.
