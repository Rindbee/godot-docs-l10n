...
Возвращает вектор, указывающий направление преломления. Для использования в графе визуального шейдера.
...
Переводится как ``refract(I, N, eta)`` на языке шейдеров, где ``I`` — вектор падения, ``N`` — вектор нормали, а ``eta`` — отношение показателей преломления.
