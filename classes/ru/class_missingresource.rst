...
Внутренний класс редактора, предназначенный для хранения данных нераспознанных ресурсов.
...
Это внутренний класс редактора, предназначенный для хранения данных ресурсов неизвестного типа (скорее всего, этот тип был предоставлен расширением, которое больше не загружено). Его нельзя вручную создать или поместить в сцену.

\ **Предупреждение:** Игнорируйте отсутствующие ресурсы, если вы не знаете, что делаете. Существующие свойства отсутствующего ресурса можно свободно изменять в коде, независимо от того, какой тип они должны иметь.
...
Имя класса, которым должен был быть этот ресурс (см. :ref:`Object.get_class()<class_Object_method_get_class>`).
...
Если установлено ``true``, то новые свойства можно добавлять поверх существующих с помощью :ref:`Object.set()<class_Object_method_set>`.
