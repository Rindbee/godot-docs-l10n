...
Вычисляет функцию Step в графе визуального шейдера.
...
Переводится в ``step(edge, x)`` на языке шейдера.
...
Порт ``x`` использует тип 2D-вектора, тогда как порт ``edge`` использует скалярный тип с плавающей точкой.
...
Порт ``x`` использует тип 3D-вектора, тогда как порт ``edge`` использует скалярный тип с плавающей точкой.
...
Представляет размер перечисления :ref:`OpType<enum_VisualShaderNodeStep_OpType>`.
