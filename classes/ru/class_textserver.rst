...
Устарело. Этот метод всегда возвращает ``1.0``.
...
Преобразует число из Западно-Арабского (0..9) в числовые системы, используемые в ``language``.

Если ``language`` не указан, будет использоваться активная локаль.
