...
Подсказки, что свойство :ref:`int<class_int>` или :ref:`float<class_float>` должно быть в диапазоне, указанном с помощью строки подсказки ``"min,max"`` или ``"min,max,step"``. Строка подсказки может опционально включать ``"or_greater"`` и/или ``"or_less"``, чтобы разрешить ручной ввод значений, выходящих соответственно за пределы максимальных или за пределы минимальных значений.

\ **Пример:** ``"-360,360,1,or_greater,or_less"``.

Кроме того, можно включить другие ключевые слова: ``"exp"`` для редактирования экспоненциального диапазона, ``"radians_as_degrees"`` для редактирования радианных углов в градусах (значения диапазона также указываются в градусах), ``"degrees"`` для указания угла и ``"hide_slider"`` для скрытия ползунка.
...
Указывает, что свойство :ref:`int<class_int>` или :ref:`String<class_String>` является перечисляемым значением, которое нужно выбрать из списка, заданного в строке подсказки.

Строка подсказки представляет собой список имен, разделенных запятыми, например ``«Hello,Something,Else»``. С обоих концов имени пробелы **не удаляются**. Для целых значений первое имя в списке имеет значение 0, следующее - 1 и так далее. Выраженные значения также можно указать, добавив к имени ``:integer``, например, ``«Zero,One,Three:3,Four,Six:6»``.
...
Указывает, что свойство :ref:`String<class_String>` может быть перечисляемым значением, которое нужно выбрать из списка, заданного в строке подсказки, например, ``"Hello,Something,Else"``.
...
Указывает, что свойство типа :ref:`int<class_int>` - это битовая маска с именованными битовыми флагами.

Текст подсказки - это разделенный запятыми список идентификаторов, таких как ``"Bit0,Bit1,Bit2,Bit3"``. Пробелы **не** удаляются ни с одной из сторон идентификатора. У первого идентификатора в списке - значение 1, у следующего - 2, дальше - 4, 8, 16, и так далее. Также можно указать явные значения, добавив ``:integer`` к идентификатору, например, ``"A:4,B:8,C:16"``. Также можно сочетать несколько флагов (``"A:4,B:8,AB:12,C:16"``).

\ **Примечание:** Значение флага должно быть как минимум ``1`` и как максимум ``2 ** 32 - 1``.

\ **Примечание:** В отличие от :ref:`PROPERTY_HINT_ENUM<class_@GlobalScope_constant_PROPERTY_HINT_ENUM>`, предыдущее явное значение не переносится на идентификатор, стоящий следом. Для подсказки с текстом ``"A:16,B,C"``, A будет 16, B - 2, C - 4.
...
Указывает, что свойство :ref:`String<class_String>` — это текст с переносами строк. При его редактировании появится поле ввода текста, в котором можно вводить переносы строк.
...
Возвращает :ref:`Object<class_Object>`, который соответствует ``instance_id``. Все объекты имеют уникальный идентификатор экземпляра. См. также :ref:`Object.get_instance_id()<class_Object_method_get_instance_id>`.
