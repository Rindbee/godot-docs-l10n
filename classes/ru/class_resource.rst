...
Глубоко дублирует этот ресурс, подобно :ref:`duplicate()<class_Resource_method_duplicate>`\ ``(true)``, с дополнительным контролем над обработкой подресурсов.

\ ``deep_subresources_mode`` должен быть одним из значений :ref:`DeepDuplicateMode<enum_Resource_DeepDuplicateMode>`.
