...
Возвращает :ref:`String<class_String>` по указанному ``index`` в массиве. Возвращает пустую строку и выводит ошибку, если доступ выходит за пределы. Отрицательные индексы не поддерживаются; они всегда будут считать значение выходящим за пределы и возвращать пустую строку. 

Это похоже на использование оператора ``[]`` (``array[index]``), за исключением того, что оператор поддерживает отрицательные индексы и вызывает остановку отладчика, если выполняется доступ за пределы.
