...
Форма цилиндра 3D, предназначенная для использования в физике. Обычно используется для предоставления формы для :ref:`CollisionShape3D<class_CollisionShape3D>`.

\ **Примечание:** Известно несколько ошибок с формами столкновений цилиндров. Вместо этого рекомендуется использовать :ref:`CapsuleShape3D<class_CapsuleShape3D>` или :ref:`BoxShape3D<class_BoxShape3D>`.

\ **Производительность:** **CylinderShape3D** быстро проверяет столкновения, но медленнее, чем :ref:`CapsuleShape3D<class_CapsuleShape3D>`, :ref:`BoxShape3D<class_BoxShape3D>` и :ref:`SphereShape3D<class_SphereShape3D>`.
