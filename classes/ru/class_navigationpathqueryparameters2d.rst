...
Изменяя различные свойства этого объекта, такие как начальная и целевая позиция, вы можете настроить запросы пути к :ref:`NavigationServer2D<class_NavigationServer2D>`.
...
Список :ref:`RID<class_RID>` регионов, которые будут исключены из запроса пути. Используйте :ref:`NavigationRegion2D.get_rid()<class_NavigationRegion2D_method_get_rid>`, чтобы получить :ref:`RID<class_RID>`, связанный с узлом :ref:`NavigationRegion2D<class_NavigationRegion2D>`.
...
Список :ref:`RID<class_RID>` регионов, которые будут включены в запрос пути. Используйте :ref:`NavigationRegion2D.get_rid()<class_NavigationRegion2D_method_get_rid>`, чтобы получить :ref:`RID<class_RID>`, связанный с узлом :ref:`NavigationRegion2D<class_NavigationRegion2D>`. Если оставить пустым, будут включены все регионы. Если регион окажется и включенным, и исключенным одновременно, он будет исключен.
...
Постобработка пути, примененная к необработанному коридору пути, найденному :ref:`pathfinding_algorithm<class_NavigationPathQueryParameters2D_property_pathfinding_algorithm>`.
...
Если ``true``, будет возвращена упрощенная версия пути с удаленными менее критическими точками пути. Степень упрощения контролируется :ref:`simplify_epsilon<class_NavigationPathQueryParameters2D_property_simplify_epsilon>`. Упрощение использует вариант алгоритма Ramer-Douglas-Peucker для прореживания точек кривой.
