...
Беззнаковый целочисленный скалярный оператор, который будет использоваться в графе визуального шейдера.
...
Применяет :ref:`operator<class_VisualShaderNodeUIntOp_property_operator>` к двум целочисленным входным значениям без знака: ``a`` и ``b``.
...
Представляет размер перечисления :ref:`Operator<enum_VisualShaderNodeUIntOp_Operator>`.
