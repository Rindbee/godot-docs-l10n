...
Объект, содержащий независимые от проекта настройки редактора. Эти настройки обычно видны в меню **Редактор > Настройки редактора**.

Имена свойств используют разделители косой черты для различения разделов. Значения настроек могут быть любого типа :ref:`Variant<class_Variant>`. Рекомендуется использовать ``snake_case`` для того, чтобы настройки редактора соответствовали самому редактору Godot.
...
\ **Примечание:** Этот класс не должен быть инстанцирован напрямую. Вместо этого, получите доступ к синглтону, используя :ref:`EditorInterface.get_editor_settings()<class_EditorInterface_method_get_editor_settings>`.
...
Минимальная ширина боковой панели палитры сеток GridMap-ов.
...
Режим поддержки доступности редактора:

- **Auto** (``0``): Поддержка специальных возможностей включена, но обновления информации о специальных возможностях обрабатываются только в том случае, если активно вспомогательное приложение (например, программа чтения с экрана или дисплей Брайля) (по умолчанию).

- **Always Active** (``1``): Поддержка специальных возможностей включена, и обновления информации о специальных возможностях всегда обрабатываются независимо от статуса вспомогательных приложений.

- **Disabled** (``2``): Поддержка специальных возможностей полностью отключена.

\ **Примечание:** Инструменты отладки доступности, такие как Accessibility Insights для Windows, Accessibility Inspector (macOS) или AT-SPI Browser (Linux/BSD), не считаются вспомогательными приложениями. Чтобы протестировать свой проект с помощью этих инструментов, используйте **Always Active**.
...
Как расположить кнопки «Отмена» и «ОК» в :ref:`AcceptDialog<class_AcceptDialog>` редактора. Разные платформы имеют разные стандартные поведения для этого, которые можно переопределить с помощью этой настройки. Это полезно, если вы используете Godot как в Windows, так и в macOS/Linux, и ваша мышечная память Godot сильнее, чем память вашей ОС.

- **Auto** следует соглашению платформы: сначала OK в Windows, KDE и LXQt, сначала Cancel в macOS и других средах рабочего стола Linux.

- **Cancel First** заставляет упорядочивать Cancel/OK.

- **OK First** заставляет упорядочивать OK/Cancel.
...
Переводы предоставляются сообществом. Если вы заметили ошибку, `внесите свой вклад в переводы редактора на Weblate! <https://contributing.godotengine.org/en/latest/documentation/translation/index.html>`__
...
Предустановленная тема редактора.
...
Если ``true``, увеличивает сенсорную область полосы прокрутки и включает более крупный перетаскиватель для разделенных контейнеров, чтобы улучшить удобство использования на устройствах с сенсорным экраном.

\ **Примечание:** По умолчанию на устройствах с сенсорным экраном установлено значение ``true``.
...
Определяет, включены ли в редакторе онлайн-функции, такие как библиотека ресурсов (Asset Library) или проверка обновлений. Отключение этих онлайн-функций помогает снизить риски нарушения конфиденциальности, запрещая редактору отправлять HTTP-запросы на веб-сайт Godot или сторонние платформы, размещающие ресурсы из библиотеки ресурсов.

Рекомендуется, чтобы плагины и скрипты инструментов редактора соответствовали этому параметру. Однако Godot не может предотвратить нарушение этого правила.
...
Пакет сертификатов TLS для использования в HTTP-запросах, сделанных в редакторе (например, из вкладки AssetLib). Если оставить пустым, будет использоваться `включенный пакет сертификатов Mozilla <https://github.com/godotengine/godot/blob/master/thirdparty/certs/ca-certificates.crt>`__.
...
Соглашение об именовании каталогов для менеджера проекта. Варианты: "Без соглашения" (имя проекта — имя каталога), "kebab-case" (по умолчанию), "snake_case", "camelCase", "PascalCase" или "Title Case".
...
Если ``true``, открывается редактор скриптов при подключении сигнала к существующему методу скрипта из дока Node.
...
Если ``true``, автоматически добавляется :doc:`статическая типизация GDScript <../tutorials/scripting/gdscript/static_typing>` (например, ``-> void`` и ``: int``) во многих ситуациях, где это возможно, в том числе при:
...
- создании переменных с префиксом :ref:`@GDScript.@onready<class_@GDScript_annotation_@onready>` путём перетаскивания узлов из дока Scene в редактор скриптов с удержанием клавиши :kbd:`Ctrl`.
...
Если ``true``, отображает обзор переменных-членов и функций текущего скрипта слева от редактора скриптов. См. также :ref:`text_editor/script_list/sort_members_outline_alphabetically<class_EditorSettings_property_text_editor/script_list/sort_members_outline_alphabetically>`.
