...
\ **Примечание:** Если вы просто хотите узнать, содержит ли строка ``what``, используйте :ref:`contains()<class_StringName_method_contains>`. В GDScript вы также можете использовать оператор ``in``.
...
Удаляет все вхождения символов в ``chars``. См. также :ref:`remove_char()<class_StringName_method_remove_char>`.
...
Возвращает индекс **последнего** вхождения ``what`` в этой строке или ``-1``, если их нет. Начало поиска можно указать с помощью ``from``, продолжая до начала строки. Этот метод является обратным :ref:`find()<class_StringName_method_find>`.
