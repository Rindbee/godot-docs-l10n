...
Переопределение размера пула потоков :ref:`WorkerThreadPool<class_WorkerThreadPool>` по умолчанию. Этот параметр используется, когда размер :ref:`ProjectSettings.threading/worker_pool/max_threads<class_ProjectSettings_property_threading/worker_pool/max_threads>` установлен в значение -1 (что является значением по умолчанию). Этот размер должен быть меньше, чем :ref:`threads/emscripten_pool_size<class_EditorExportPlatformWeb_property_threads/emscripten_pool_size>`, иначе могут возникнуть взаимоблокировки.

При использовании потоков этот размер должен быть достаточно большим для поддержки функций, требующих наличия выделенного потока, таких как :ref:`ProjectSettings.physics/2d/run_on_separate_thread<class_ProjectSettings_property_physics/2d/run_on_separate_thread>` или :ref:`ProjectSettings.rendering/driver/threads/thread_model<class_ProjectSettings_property_rendering/driver/threads/thread_model>`. В общем случае рекомендуется, чтобы этот размер был не менее 4 и был как минимум на 2 или 3 меньше, чем :ref:`threads/emscripten_pool_size<class_EditorExportPlatformWeb_property_threads/emscripten_pool_size>`.
